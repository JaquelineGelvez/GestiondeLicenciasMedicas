...
# =====================================================================
# BLOQUE 4: SOLICITUD Y VALIDACIÓN DEL CERTIFICADO MÉDICO (MÁXIMO 3 INTENTOS)
# =====================================================================
# Se le solicita al usuario la ruta del archivo del certificado médico.
# Se valida que el archivo exista en el sistema y que tenga una extensión
# válida (.pdf, .jpg, .jpeg, .png).
# Tiene hasta 3 intentos antes de que el programa finalice.
intentos_cert = 0
extensiones_validas = (".pdf", ".jpg", ".jpeg", ".png")
while True:
    ruta_certificado = (
        input(
            "\nIngrese la ruta del archivo de su certificado médico "
            "(ej: certificado.pdf): "
        )
        .strip()
    )
    # Validar si el archivo existe y tiene extensión permitida
    existe = os.path.exists(ruta_certificado)
    tiene_extension = ruta_certificado.lower().endswith(extensiones_validas)

    if existe and tiene_extension:
        nombre_archivo = os.path.basename(ruta_certificado)
        print(
            f"Certificado '{nombre_archivo}' cargado y "
        ...
        elif not tiene_extension:
            print(
                "Error: Extensión inválida. Formatos permitidos: "
